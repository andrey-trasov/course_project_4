...
class Vacancy:
    def __init__(self, hh_vacancies):
        ...
        self.currency = self.currency(hh_vacancies)
    ...
    def get_product_price(hh_vacancies):
        if 'salary' in hh_vacancies and hh_vacancies['salary'] is not None:
    ...
    @staticmethod
    def currency(hh_vacancies):
        if 'salary' in hh_vacancies and hh_vacancies['salary'] is not None:
            return hh_vacancies.get('salary', 'валюта не указана').get('currency', 'валюта не указана')  # валюта
    ...
    def cast_to_object_list(cls, hh_vacancies):
        ...
        for vacancy in hh_vacancies:
            print(vacancy)
            returned_list.append(cls(vacancy))
